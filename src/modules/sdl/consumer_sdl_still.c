...
mlt_consumer consumer_sdl_still_init(mlt_profile profile,
...
static inline void display_1(
    SDL_Surface *screen, SDL_Rect rect, uint8_t *image, int width, int height)
{
...
static int consumer_play_video(consumer_sdl this, mlt_frame frame)
{
    ...
    if (SDL_GetVideoSurface()) {
        ...
        while (SDL_PollEvent(&event)) {
            mlt_events_fire(this->properties,
                            "consumer-sdl-event",
                            mlt_event_data_from_object(&event));
            ...
            case SDL_KEYDOWN: {
                mlt_producer producer = mlt_properties_get_data(properties,
                                                                ...
                                                                NULL);
                char keyboard[2] = " ";
                void (*callback)(mlt_producer, char *)
                    = mlt_properties_get_data(properties, "transport_callback", NULL);
                if (callback != NULL && producer != NULL && event.key.keysym.unicode < 0x80
    ...
    if (changed == 0 && this->last_position == mlt_frame_get_position(frame)
    ...
    if (image != NULL) {
        ...
        if (rescale != NULL && strcmp(rescale, "none")) {
            double this_aspect = display_ratio
                                 / ((double) this->window_width / (double) this->window_height);
    ...
    while (this->running) {
        ...
        if (this->running && frame != NULL) {
            if (preview_off == 0) {
                ...
                int width = this->width;
                uint8_t *image = NULL;
                mlt_image_format preview_format = mlt_properties_get_int(properties,
                                                                         "preview_format");
