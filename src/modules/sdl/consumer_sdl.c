    ...
    *duration = ((samples * 1000) / frequency);
    ...
    if (init_audio == 1) {
        ...
        if (SDL_OpenAudio(&request, &got) != 0) {
            mlt_log_error(MLT_CONSUMER_SERVICE(self),
    ...
    if (self->running && display_off == 0) {
        ...
        if (SDL_GetVideoSurface()) {
            ...
            while (SDL_PollEvent(&event)) {
                mlt_events_fire(self->properties,
                                "consumer-sdl-event",
        ...
        if (self->running && (!SDL_GetVideoSurface() || changed)) {
            ...
            pthread_mutex_lock(&mlt_sdl_mutex);
            SDL_Surface *screen = SDL_SetVideoMode(self->window_width,
                                                   self->window_height,
                                                   self->bpp,
                                                   self->sdl_flags);
            if (consumer_get_dimensions(&self->window_width, &self->window_height))
                screen = SDL_SetVideoMode(self->window_width,
                                          self->window_height,
                                          self->bpp,
                                          self->sdl_flags);
            pthread_mutex_unlock(&mlt_sdl_mutex);
        ...
        if (self->running && SDL_GetVideoSurface() && self->sdl_overlay == NULL) {
            SDL_SetClipRect(SDL_GetVideoSurface(), &self->rect);
            self->sdl_overlay = SDL_CreateYUVOverlay(width,
                                                     height,
                                                     (vfmt == mlt_image_yuv422 ? SDL_YUY2_OVERLAY
                                                                               : SDL_IYUV_OVERLAY),
                                                     SDL_GetVideoSurface());
        }
...
static void *consumer_thread(void *arg)
{
    ...
    int terminate_on_pause = mlt_properties_get_int(MLT_CONSUMER_PROPERTIES(consumer),
                                                    "terminate_on_pause");
    int terminated = 0;
