...
typedef struct SA_API PolarPoint
{
...
*/
SA_API float DegreesToRadians(float fDegrees);

/**
    Convert radians to degrees.
*/
SA_API float RadiansToDegrees(float fRadians);

/**
    Get the number of BFormat components for a given BFormat configuration.
*/
SA_API unsigned OrderToComponents(unsigned nOrder, bool b3D);
...
*/
SA_API unsigned OrderToComponentPosition(unsigned nOrder, bool b3D);

/**
    Get the recommended minimum speakers needed to decode a BFormat stream of
    a given configuration.
*/
SA_API unsigned OrderToSpeakers(unsigned nOrder, bool b3D);

/**
    Get the label for a given index component in a BFormat stream.
*/
SA_API char ComponentToChannelLabel(unsigned nComponent, bool b3D);
